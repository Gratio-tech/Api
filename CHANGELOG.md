...
### Исправлено
- На windows путь вида `C:\` в хелпере `isUrl` резолвился в корректный протокол, поэтому схему нужно проверять точно.


## [3.1.5] - 2026-08-19

### Исправлено
- Простая проверка `response.redirected` на GitHub всегда выдавала ошибку даже на публичных файлах, так как
  Github возвращает для них редирект на `raw.githubusercontent.com/...`. Реализована более корректная проверка.
- Тестирование нового пайплайна - репозиторий добавлен в Trusted Publisher пакета


## [3.1.4] - 2026-08-19

### Исправлено
- Исправлена секция "bin" в package.json


## [3.1.3] - 2026-08-19

### Добавлено
- Добавлена нормализация пути переданного в аргумент --link

### Исправлено
- Если линк и токен не найдены в process.env, то до проверки аргументов CLI даже не доходило
- Теперь если линк на openapi-схему не найден, будет выбрасываться ошибка (нужно чтобы пайплайны не выполнялись дальше)
